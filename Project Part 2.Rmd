...
Part 1 exploratory analysis revealed an upward trend in the proportion of total points scored from three-point field goals in the NBA from 2019 to 2022, increasing from 31.6% to 33.5%. However, exploratory analysis alone cannot determine whether this change represents a statistically significant shift or natural variation.

**Has the proportion of total points scored from three-point field goals increased in the NBA from 2019 to 2022?**

This research question was selected because it addresses a fundamental strategic shift in modern basketball and showed a clear directional trend. The question compares two distinct time periods (2019 vs. 2022) and involves a proportion that can be precisely measured.

The data come from Basketball Reference (https://www.basketball-reference.com/) and include player-level per-game statistics for the 2018-19 season (labeled 2019) and 2021-22 season (labeled 2022), representing all NBA players who appeared in games. The data provide complete league-wide coverage of scoring events. By aggregating player-level statistics, we calculate the total points from three-pointers and total points overall for each season, enabling direct proportion comparison. Data were cleaned to remove missing values for critical variables and players with zero field goal attempts.

## Methods

### Test Selection

A **two-sample z-test for proportions** was selected. This test compares two population proportions (p₁ for 2019 and p₂ for 2022), where each represents the proportion of total points from three-point field goals. This is a two-sample test comparing proportions from independent populations (2019 and 2022 NBA seasons), directly addressing whether the proportion increased from 2019 to 2022.
...
Several limitations should be considered: (1) **Temporal scope**: The analysis compares only two seasons, providing a limited view of long-term trends. (2) **COVID-19 impact**: The 2020 and 2021 seasons were affected by the pandemic, which may have influenced the natural evolution of offensive strategies. (3) **Aggregation level**: The analysis aggregates player-level data to league-wide proportions, which may obscure team-specific or player-specific variations. (4) **Causal inference**: While the test establishes statistical significance, it does not establish causality or identify specific factors driving this change. (5) **Population vs. sample**: The data represent the complete population of NBA players in these seasons, but we treat them as samples for statistical inference.
